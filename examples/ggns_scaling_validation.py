...
def main() -> None:
    ...
    for target_name, dimension in target_grid:
        ...
        if target_spec.name == "correlated_gaussian":
            ...
            print(
                "Reference note: prior is an indicator on [-6, 6]^d "
                "(unnormalized), so logZ=log P(X in box)."
            )
        ...
        for sampler in samplers:
            ...
            mean_logz_by_sampler[sampler] = summarize(
                sampler, results, reference_logz=reference_logz
            )
